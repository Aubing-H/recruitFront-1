...
class MyReq extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    componentDidMount() {
        ...
        let url = "http://localhost:8080/tokenReq/myAll?req_username="+user.username;
        fetch(url,{
            method:'GET',
            headers:myHeaders,
            mode:'cors'
                    ...
                    {this.state.callingReqs.map(item => (
                        <Row className="my-item" key={item.req_id}>
                            <Card title={<Link to={{pathname:'/TokenDetails',query:{token_id:item.token_id}}}>点击此处查看召集令信息</Link>} bordered={false}>
                                ...
                                >
                                    <Link to={{pathname:'/recevier/req_modify',query:{token_req:item}}}>修改</Link>
                                </Button>
                                {/* </Link> */}
                                <Button
                                    type="primary"
                                ...
                                <Button
                                    type="primary"
                                    style={{ margin: "0 10px" }}
                                    onClick={e => this.onCancel(e, item)}
                                    disabled={item.state!=='waitprocess'}
                                >
                                    取消
                                </Button>
