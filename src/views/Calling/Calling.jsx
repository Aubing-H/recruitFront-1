...
class Calling extends React.Component {
  ...
  onDelete = (e, item) => {
    ...
    const arraytotal = this.state.total;
    for (i = 0; i < arraytotal.length; i++) {
      if (arraytotal[i].token_id === call_id) {
        break;
      }
    }
    if (arraytotal.length > 0) {
      arraytotal.splice(i, 1); //删除
      this.setState({ total: arraytotal });
    }
  };
    ...
    return (
      <Layout className="calling">
        <div className="calling-head">
          ...
          <Dropdown.Button overlay={menu} className="search" onClick={this.chooseType}>
